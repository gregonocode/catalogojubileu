...
function formatNumberToBRLText(n: number) {
  // pra preencher o input com algo tipo "199,9" / "199,90"
...
export default function ProdutoEditarPage() {
  ...
  // imagem atual do produto (caso não troque)
  ...
  }, [produtoId, empresaId, nome, precoTxt, estoqueTxt, saving]);
  ...
  async function uploadImagem(empresaId: string, produtoNome: string, f: File) {
    ...
    const { error: upErr } = await supabaseClient.storage.from("produtos").upload(filePath, f, {
      cacheControl: "3600",
      upsert: false,
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // produto (garante que pertence à empresa)
        ...
        if (!prod) {
          toast.error("Produto não encontrado (ou você não tem acesso).");
          return;
        }

        const p = prod as ProdutoRow;

        if (!mounted) return;
        setNome(p.nome ?? "");
        setDescricao(p.descricao ?? "");
        setPrecoTxt(formatNumberToBRLText(Number(p.preco ?? 0)));
        setEstoqueTxt(String(Number(p.estoque ?? 0)));
        setCategoriaId(p.categoria_id ?? ((cats ?? [])[0]?.id ?? ""));
        setAtivo(Boolean(p.ativo ?? true));
        setImagemAtualUrl(p.imagem_url ?? null);

        // não mexe no file/preview aqui (só muda quando usuário selecionar)
        setFile(null);
        setPreviewUrl(null);
      } catch (err) {
        console.error(err);
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      let imagem_url = imagemAtualUrl ?? null;

      // se escolheu uma nova imagem, faz upload e substitui a URL
      if (file) {
      ...
      setImagemAtualUrl(imagem_url);
      setFile(null);
      setPreviewUrl(null);
    } catch (err: unknown) {
      console.error(err);
      toast.error("Não foi possível salvar. Verifique e tente novamente.");
    } finally {
      setSaving(false);
    }
  }

  const imageToShow = previewUrl || imagemAtualUrl;
      ...
      <section className="rounded-3xl border border-black/10 bg-white p-6 shadow-[0_10px_30px_-20px_rgba(0,0,0,0.25)]">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div className="text-lg font-semibold text-black">Editar produto</div>
            <div className="mt-1 text-xs text-black/55">
              Atualize nome, preço, estoque, categoria e imagem.
            </div>
          </div>
